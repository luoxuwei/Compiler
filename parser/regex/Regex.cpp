...
void Regex::preProcessExpr(const std::string &regex) {
...
std::string & Regex::extracMacroNameFromInput(LexerBuffer &lexer) {
    char c = lexer.advance();
    std::string name;
    while (c != '}' && !isspace(c)) {
        name.push_back(c);
...
std::string Regex::expandMacro(const std::string &name) {
...
bool Regex::checkInQuoted(std::string &macroContent, int curlyBracesBegin, int curlyBracesEnd) {
