...
void * ArgsExecutor::Execute(ICodeNode *root) {
    ...
    vector<Symbol *> *symList = new vector<Symbol *>(), *argSymbolList;
    ...
    switch (production) {
        case GrammarInitializer::NoCommaExpr_TO_Args:
            ...
            symList->push_back((Symbol *) child->getAttribute(ICodeNode::SYMBOL));
            break;
        case GrammarInitializer::NoCommaExpr_Comma_Args_TO_Args:
            ...
            argSymbolList = (vector<Symbol *> *) child->getAttribute(ICodeNode::SYMBOL);

            argList->insert(argList->end(), valueList->begin(), valueList->end());
            symList->insert(symList->end(), argSymbolList->begin(), argSymbolList->end());
            break;
    }
    root->setAttribute(ICodeNode::VALUE, new Value(argList));
    root->setAttribute(ICodeNode::SYMBOL, symList);
    return root;
}